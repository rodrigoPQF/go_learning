...
func converg() {
	par := make(chan int)
	impar := make(chan int)
	coverge := make(chan int)

	go envia(par, impar)
	go receiver2(par, impar, coverge)
...
func outra(canal, canal2 chan int) {
	var wg sync.WaitGroup
...
func divergencia2() {
	...
	functions := 20
	go manda2(20, canal1)
	go outra2(functions, canal1, canal2)
...
func outra2(functions int, canal, canal2 chan int) {
	var wg sync.WaitGroup
